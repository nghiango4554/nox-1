...
def run_full_auto(confirm_phrase=None, qa=False, dry_run=False, max_articles=None, priority_cids=None,
                  auto_only=False, skip_decided=False, candidate_ids=None, mode_tag=None):
    ...
    candidate_ids: nếu truyền → xử lý ĐÚNG list này theo thứ tự (P8 all-in-one), MỌI bài full pipeline
    (không short-circuit DEFER); bỏ qua lane/auto_only/skip_decided/priority. mode_tag: nhãn run."""
    migrate()
    cfg = load_config()
    cb = auto.cb_state()
    if cb.get("open"):
    ...
    if candidate_ids:
        # P8 — xử lý đúng list ordered, mọi bài AUTO pipeline (gates tự quyết outcome)
        bycid = {c["id"]: c for c in queue_candidates()}
        q = []
        for x in candidate_ids:
            c = bycid.get(int(x))
            if c:
                c["_lane"] = "AUTO"  # ép full pipeline, KHÔNG short-circuit
                q.append(c)
    else:
