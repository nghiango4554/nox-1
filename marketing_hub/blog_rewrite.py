...
def latest_draft_for_candidate(cid):
    conn = db.get_conn()
    # BỎ QUA draft 'p0_preview' (P9 executor, out-of-band) — luồng apply/plagiarism KHÔNG được nhặt nhầm
    r = conn.execute("SELECT * FROM blog_rewrite_drafts WHERE candidate_id=? AND approval_status!='p0_preview' "
