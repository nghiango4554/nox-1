# -*- coding: utf-8 -*-
"""Format + sync bài 'Sửa máy tính Quận 7' lên blog (1002398567). Blog body-only, bù internal link."""
import re, time, urllib.parse
from pathlib import Path
from bs4 import BeautifulSoup
import blog_rewrite_apply as ap

BLOG_ID, ART_ID = 1000906526, 1002398567
...
for a in root.find_all("a"):
    ...
    if (not real) or not txt:
        a.unwrap()
    else:
        a["href"] = real; a["style"] = LK
...
for n in list(root.children):
    ...
    if nm == "h2":
        ht = n.get_text().strip()
        if not seen and "Sửa máy tính Quận 7 tại Sintech" in ht:
            seen = True; continue
        if "Mục lục" in ht:
            skip = True; continue
...
# bù internal link collection ở mục "Nâng cấp PC và laptop"
def mklink(p_text_parts):
    p = soup.new_tag("p"); p["style"] = S["p"]
    for part in p_text_parts:
        if isinstance(part, tuple):
            a = soup.new_tag("a", href=part[1]); a.string = part[0]; a["style"] = LK; p.append(a)
        else:
            p.append(part)
    return p
for h in cont.find_all("h2"):
    if "Nâng cấp PC và laptop" in h.get_text():
        lp = mklink(["Khi nâng cấp, bạn có thể tham khảo ", ("laptop", "/collections/laptop"), ", ",
                     ("màn hình máy tính", "/collections/man-hinh-may-tinh"), " hoặc ",
                     ("linh kiện PC", "/collections/linh-kien-may-tinh"), " tại Sintech."])
        nxt = h.find_next_sibling("h2")
        (nxt.insert_before(lp) if nxt else cont.append(lp))
        break
...
# internal link Sintech đầu + cuối
top = '<p style="%s">Trung tâm <a href="https://sintech.vn" style="%s">Sửa máy tính Quận 7 tại Sintech</a> nhận kiểm tra, sửa PC và laptop lấy ngay, có báo giá rõ trước khi sửa.</p>' % (S["p"], LK)
bot = '<p style="%s">Liên hệ <a href="https://sintech.vn" style="%s">Sintech</a> để được kiểm tra và báo giá sửa máy. Hotline 0911 713 000 · 457 Trần Xuân Soạn, Q7, TP.HCM.</p>' % (S["p"], LK)
final = top + body_html + bot

# backup + sync blog (body-only, giữ title)
code, art = ap._fetch_live_article(BLOG_ID, ART_ID)
Path("data/_suaq7_OLD_backup_%s.html" % time.strftime("%Y%m%d_%H%M%S")).write_text(art.get("body_html") or "", encoding="utf-8")
...
print("LIVE: %d ký tự · %d bảng · h2-styled=%d · link đỏ=%d · sintech.vn link=%d · escaped=%d · title: %s" %
      (len(vb), vb.count("<table"), vb.count("font-size:17pt"), vb.count("#e74c3c"),
       vb.count('href="https://sintech.vn"'), vb.count("&lt;a"), v.get("title")[:40]))
