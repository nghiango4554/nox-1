# -*- coding: utf-8 -*-
"""Format + sync bài blog 'Bảng mã lỗi Mainboard Huananzhi X99' (blog 1000906526 / art 1002411453).
...
BLOG_ID, ART_ID = 1000906526, 1002411453
TITLE_H2 = "Bảng mã lỗi main Huananzhi X99"   # H2 tiêu đề trong doc -> bỏ (template đã có title)
...
top = ('<p style="%s"><a href="https://sintech.vn" style="%s">Sintech</a> tổng hợp bảng mã lỗi '
       'mainboard Huananzhi X99 (X79) cùng cách đọc mã LED debug và hướng xử lý khi PC Xeon không lên hình.</p>'
       % (S["p"], LK))
bot = ('<p style="%s">Nếu main X99 báo lỗi khó xác định hoặc cần kiểm tra phần cứng, liên hệ '
       '<a href="https://sintech.vn/collections/sua-chua-pc-laptop" style="%s">Sintech</a> để được hỗ trợ. '
...
Path("data/_x99_OLD_backup_%s.html" % time.strftime("%Y%m%d_%H%M%S")).write_text(art.get("body_html") or "", encoding="utf-8")
