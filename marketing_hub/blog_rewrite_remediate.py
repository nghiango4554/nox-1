...
def canary_prep():
    ...
    # HOLD (P5G-4): bài có facts time-sensitive/unsupported chưa xử lý
    try:
        hold_map = json.loads((_Path(__file__).parent / "state" / "_canary_hold.json").read_text(encoding="utf-8"))
    except Exception:
        hold_map = {}
    ready, review, hold = [], [], []
    ...
    for cid in drafted:
        ...
        if str(cid) in hold_map:
            rec["rollout_status"] = hold_map[str(cid)].get("rollout_status", "HOLD")
            rec["recommended_action"] = hold_map[str(cid)].get("recommended_action", "")
            hold.append(rec)
        elif brand_ok and html_ok and ps["conflict_status"] == "SAFE_TO_APPLY" and (ov or 0) <= OVERLAP_MAX:
            rec["rollout_status"] = "READY_EVERGREEN"
            ready.append(rec)
        else:
    ...
    return {
        ...
        "canary_ready": ready, "review_required": review, "hold": hold,
        "selected_canary": [r["candidate_id"] for r in ready[:2]],
        "note": "canary-ready = draft + gate ALLOW + conflict SAFE + overlap≤12% + brand/HTML sạch + KHÔNG HOLD. "
                "Evergreen (không số liệu time-sensitive) ưu tiên. Flags live VẪN KHÓA.",
    }
