...
def _p5_blocked():
    return jsonify({"ok": False, "phase": "P5",
                    "error": "Apply Haravan chưa bật. Không có thay đổi nào được gửi lên website."}), 501
...
def api_image_plan(did):
    return jsonify(br.image_plan(int(did), refresh=False))


def api_image_plan_refresh(did):
    return jsonify(br.image_plan(int(did), refresh=True))
