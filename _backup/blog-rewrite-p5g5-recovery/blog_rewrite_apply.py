...
def apply_draft_body_only(draft_id, confirm_phrase="", fields=None,
    ...
    # ─── post-PUT verify GET (semantic — chống mismatch giả do Haravan normalize) ───
    import blog_rewrite_verify as _v
    code2, art2 = _get_live(blog_id, article_id)
    after_hash = _hash_body((art2 or {}).get("body_html") or "")
    if code2 != 200:
        verify = "READ_AFTER_WRITE_FAILED"
        verify_detail = "READ_AFTER_WRITE_FAILED"
    else:
        cmp = _v.compare_article_signatures(
            _v.build_article_verify_signature(d.get("draft_body_html") or ""),
            _v.build_article_verify_signature((art2 or {}).get("body_html") or ""))
        verify = "VERIFIED" if cmp["status"].startswith("VERIFIED") else "VERIFY_MISMATCH"
        verify_detail = cmp["status"]
    conn = db.get_conn()
    conn.execute("UPDATE blog_rewrite_drafts SET applied_at=datetime('now'), applied_draft_hash=?, "
                 ...
                 (draft_hash, nonce, json.dumps({"verify": verify, "verify_detail": verify_detail, "http": status,
