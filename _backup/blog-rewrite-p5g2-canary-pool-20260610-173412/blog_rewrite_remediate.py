...
def canary_prep():
    ...
    for cid in drafted:
        ...
        ready.append({
            ...
            "overlap": q.get("normalized_5gram_overlap"),
            "verify_preview": ps["conflict_status"], "html_safety": "PASS",
        })
    ready.sort(key=lambda x: (0 if x["approved_local"] else 1, x["image_count"], x["table_count"],
                              (x["gsc_clicks_28d"] or 0)))
    return {
        ...
        "note": "Chỉ chọn bài có draft AI thật + gate ALLOW + chưa apply. SAFE_NOW không draft cần generate trước.",
    }
