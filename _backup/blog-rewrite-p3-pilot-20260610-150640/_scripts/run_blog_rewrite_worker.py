...
def run(jid):
    ...
    for cid in cids:
        ...
        try:
            ...
            br.record_event("generate_started", candidate_id=cid, job_id=jid)
            time.sleep(0.2)  # mô phỏng thời gian xử lý, KHÔNG network
            draft_id, ver = br.save_mock_draft(cand, jid)
            br._set_candidate_status(cid, "draft_ready")
            br.record_event("generate_completed", candidate_id=cid, draft_id=draft_id, job_id=jid,
                            detail={"version": ver, "mock": True})
            completed += 1
